...
class AuthInterceptors extends InterceptorsWrapper {
  ...
  Future onError(DioError e) async {
    ...
    if (e.response?.statusCode == 401) {
      ...
      return refreshLogin().then((d) {
        //update csrfToken
        options.headers.addAll({"auth": d});
      }).whenComplete(() {
        dio.unlock();
  ...
  Future onResponse(Response response) async {
    ...
    await prefs.save("token", token);
    return response;
  }

  Future<String> refreshLogin() async {
    final newDio = CustomDio();
    var userData = LoginModel.fromJson(await prefs.read("userData"));
    final response = await newDio.post("/login", data: userData.toJson());
    var token = response.data["token"];
    await prefs.save("token", response.data["auth"]);
    return token;
  }
}
